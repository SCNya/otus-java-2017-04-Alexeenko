...
import com.otus.alexeenko.database.services.custom.service.MsgDbService;
...
public class CustomService implements DataBaseService {
    ...
    public CustomService(MsgConnection msgServer) {
        check();
        ...
        msgService = new MsgDbService();
        msgService.setMsgServer(msgServer);
    }

    //For connect to own exist DataBase
    public CustomService(MsgConnection msgServer, JdbcDataSource dataSource) {
        check();
        server = null;
        context = new ClassPathXmlApplicationContext("DataBaseBeans.xml");
        myCache = ((MyCache) context.getBean("cache"));
